...
class TestDocumentProcessor:
    ...
    def test_process_upload_nonexistent_path(self, mocker):
        """Test processing non-existent path raises error."""
        mock_exists = mocker.patch("src.document_to_anki.core.document_processor.Path.exists")
    ...
    def test_process_upload_single_file_success(self, mocker):
        """Test successful processing of a single file."""
        # Setup mocks
        mock_exists = mocker.patch("src.document_to_anki.core.document_processor.Path.exists")
        mock_is_file = mocker.patch("src.document_to_anki.core.document_processor.Path.is_file")
        mock_exists.return_value = True
        mock_is_file.return_value = True

        # Mock file handler and text extractor
        mocker.patch.object(self.processor.file_handler, "validate_file_type", return_value=True)
        mocker.patch.object(self.processor.text_extractor, "extract_text", return_value="Sample text content")
    ...
    def test_process_upload_file_validation_failure(self, mocker):
        ...
        mocker.patch.object(self.processor.file_handler, "validate_file_type", return_value=False)
        with pytest.raises(DocumentProcessingError, match="Unsupported file type"):
            self.processor.process_upload("test.xyz")

    def test_process_upload_text_extraction_failure(self, mocker):
    ...
    def test_process_upload_folder_success(self, mocker):
        """Test successful processing of a folder."""
        mock_exists = mocker.patch("src.document_to_anki.core.document_processor.Path.exists")
        mock_is_file = mocker.patch("src.document_to_anki.core.document_processor.Path.is_file")
        mock_is_dir = mocker.patch("src.document_to_anki.core.document_processor.Path.is_dir")
        mock_exists.return_value = True
    ...
    def test_validate_upload_path_valid_file(self, mocker):
...
class TestDocumentProcessingResult:
    ...
    def test_success_property(self):
        ...
        result = DocumentProcessingResult(
            text_content="Test", source_files=["test.txt"], file_count=1, total_characters=4
        )
