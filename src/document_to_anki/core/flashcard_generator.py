...
class FlashcardGenerator:
    ...
    def preview_flashcards(
        ...
        for card in cards_to_preview:
            ...
            source_preview = (card.source_file or "Unknown")[:17] + "..." if card.source_file and len(card.source_file) > 20 else (card.source_file or "Unknown")
        ...
        for i, card in enumerate(cards_to_preview, 1):
            ...
            card_content = Text()
            card_content.append(f"Question: ", style="bold cyan")
            card_content.append(f"{card.question}\n\n", style="white")
            card_content.append(f"Answer: ", style="bold green")
            card_content.append(f"{card.answer}\n\n", style="white")
            card_content.append(f"Source: ", style="bold blue")
            ...
            panel = Panel(
                card_content,
                title=panel_title,
                border_style="bright_blue" if card.validate() else "red",
                expand=False
            )
            console.print(panel)
